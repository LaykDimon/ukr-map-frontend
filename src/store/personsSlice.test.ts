...
describe("personsSlice", () => {
  ...
  it("should return the initial state", () => {
    expect(personsReducer(undefined, { type: "unknown" })).toEqual(
      initialState,
    );
  });
  ...
  describe("removePerson", () => {
    ...
    it("should persist removedIds to localStorage", () => {
      ...
      expect(JSON.parse(localStorage.getItem("removedIds") || "[]")).toContain(
        "test-uuid-1",
      );
    });
  });
  ...
  describe("restorePerson", () => {
    it("should remove id from removedIds", () => {
      const stateWithRemoved = {
        ...initialState,
        removedIds: ["test-uuid-1", "test-uuid-2", "test-uuid-3"],
      };
      const state = personsReducer(
        stateWithRemoved,
        restorePerson("test-uuid-2"),
      );
